...
也不决定 resume 路由。
正式 API 和未配置模式的工具入口默认使用真实搜索（`RENTAL_DEMO_MODE=live`）；网络或平台访问失败不会自动回退到夹具。
只有显式设置 `RENTAL_DEMO_MODE=offline` 才使用离线房源；完全离线回归还应设置 `MAP_PROVIDER=fake`。
已有私有配置不会被安装脚本覆盖：旧版本若配置了 `offline`，请手动改为 `live`，地图使用 `MAP_PROVIDER=amap`，重启后端并新建会话测试。
